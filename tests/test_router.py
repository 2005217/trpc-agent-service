...
def test_session_id_isolation_by_tenant_and_channel():
    ...
    assert SessionRouter.session_id("t1", "web", "user1") != SessionRouter.session_id("t1", "wecom", "user1")


def test_group_chat_isolated():
    single = SessionRouter.session_id("t1", "wecom", "user1")
    group = SessionRouter.session_id("t1", "wecom", "user1", chat_id="room9")
    assert single != group
