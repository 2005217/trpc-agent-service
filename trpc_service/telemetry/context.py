...
生成/透传 trace_id 并注入 AgentContext 元数据；
OTel provider 初始化见 telemetry.setup（T3）。
"""
...
from trpc_service.filter.context import META_CHANNEL
from trpc_service.filter.context import META_SESSION
from trpc_service.filter.context import META_TENANT
from trpc_service.filter.context import META_TRACE
from trpc_service.filter.context import META_USER
