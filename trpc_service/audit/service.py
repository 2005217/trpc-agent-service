...
生产级形态（②决策二的生产级路线）：
- emit() 只进内存缓冲（微秒级，不阻塞请求热路径）
- 后台任务按间隔批量 INSERT（batch_size 上限）
- SQL 写失败 / 缓冲溢出 → 落 JSONL 文件兜底（审计不丢）
- 崩溃窗口说明：缓冲在内存，进程崩溃时未落盘记录丢失；
  更高可靠性可升级为 Redis List 先行缓冲（演进注记）

文件后端保留完整能力（emit/tail/cleanup），作为无 SQL 场景的一等公民。
"""
...
class AuditService:
    ...
    def emit(self, event: AuditEvent) -> AuditEvent:
        ...
        缓冲统一保存 JSON 安全形态（created_at 为 ISO 字符串）——
        溢出溢到文件、SQL 失败降级到文件，都无需再转换；
        datetime 转换只在 flush 落库边界做一次。
        """
