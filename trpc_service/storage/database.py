...
与框架 Session/Memory 服务各自的连接池互不干涉——共享的是平台表，
隔离的是框架数据。生产走 Alembic 迁移建表；测试可注入 SQLite 内存库。
"""
...
from trpc_service.storage.tables import Base
...
class Database:
    ...
    def create_all(self) -> None:
        ...
        生产路径用 Alembic 迁移（migrations/）；本方法仅供单元测试
        （SQLite 注入）和无 Alembic 的极简场景使用。
        """
