...
### IM 软件接入
- **第一阶段**：通过 Web UI（FastAPI 页面）与 Agent 通信，实现基础的聊天交互。
- **第二阶段**：接入企业微信，实现 Channel Adapter，支持企业微信消息收发。
...
3.必须说明 Web UI 与 Agent 的通信方式，以及第二阶段企业微信接入的预留设计。
4.必须说明 Redis 和 SQL 两种后端的数据存储和同步策略，以及租户如何选择。
